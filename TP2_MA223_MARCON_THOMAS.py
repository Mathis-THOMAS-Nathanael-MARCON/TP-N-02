...
def ResolutionCholesky(L, U, B):
    ...
    for i in range(0,ligne):
        matrice_y[i] =  matrice_LB[i][ligne]
        
        for k in range(0,i):
            matrice_y[i] =  matrice_y[i] - matrice_LB[i][k]*matrice_y[k]
        matrice_y[i] = matrice_y[i]/matrice_LB[i][i]
    
    matrice_Ux = np.c_[U, matrice_y]
    sol = ResolutionSystTrisup(matrice_Ux)
    return sol

def MethodeCholesky(matrice_A, matrice_B):
    matrice_A = Cholesky(matrice_A)
    sol = ResolutionCholesky(matrice_A, matrice_A.T, matrice_B)
    return sol

##### Linalg.Cholesky #####

def MethodelinalgCholesky(matrice_A, matrice_B):
    matrice_A = np.linalg.cholesky(matrice_A)
    sol = ResolutionCholesky(matrice_A, matrice_A.T, matrice_B)
    return sol
...
def Graph_temps():
    ...
    for i in range(250,1000,50):
       ...
       #print(durée2)
        ...
        time1_6 = time.time()
        MethodeCholesky(matrice_F, matrice_B)
        time2_6 = time.time()
        durée6 = time2_6 - time1_6
    
        time1_7 = time.time()
        MethodelinalgCholesky(matrice_G, matrice_B)
        time2_7 = time.time()
        durée7 = time2_7 - time1_7
    ...
    pp.plot (x, y6, "o--", color = 'c', label = 'Cholesky')
    pp.plot (x, y7, "o--", color = 'k', label = 'linalg.Cholesky')
    pp.yscale('log')
    pp.xscale('log')
...
def Graph_erreur():
    ...
    for i in range(250,1000,50):
        ...
        X6 = MethodeCholesky(matrice_F, matrice_B)
        Xabs6 = np.linalg.norm(np.dot(matrice_F, X6)-np.ravel(matrice_B))
        
        X7 = MethodelinalgCholesky(matrice_G, matrice_B)
        Xabs7 = np.linalg.norm(np.dot(matrice_G, X7)-np.ravel(matrice_B))
